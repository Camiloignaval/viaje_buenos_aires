...
function abstain(
  ...
  return Object.freeze({
    ...
    requiredCapabilities: freezeArray(rule.requiredCapabilities),
    sourceModules: freezeArray(rule.requiredModules),
    outcome: "abstain",
    reasonCode,
...
export function createContextDecisionRun(input: DecisionInput, dependencies: DecisionDependencies): ContextDecisionRun {
  ...
  const selected: ActDecision | null = selectedEvaluation
    ? Object.freeze({
        ...
        requiredCapabilities: selectedEvaluation.requiredCapabilities,
        sourceModules: selectedEvaluation.sourceModules,
