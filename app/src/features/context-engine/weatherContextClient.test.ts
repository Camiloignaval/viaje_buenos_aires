...
const request = { latitude: -34.6037, longitude: -58.3816, timezone: "America/Argentina/Buenos_Aires", localDate: "2026-10-03" };
const snapshot = {
  value: {
...
describe("fetchWeatherContext", () => {
  it("envía las coordenadas solo en el body autenticado y devuelve el contrato normalizado", async () => {
    ...
    await expect(fetchWeatherContext(request)).resolves.toEqual(snapshot);
  ...
  it("descarta respuestas runtime incompletas sin filtrar payload parcial", async () => {
    globalThis.fetch = vi.fn(async () => new Response(JSON.stringify({ ...snapshot, value: { condition: "clear" } }), { status: 200 }));
